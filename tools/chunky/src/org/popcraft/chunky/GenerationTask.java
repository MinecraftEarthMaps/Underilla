...
public class GenerationTask implements Runnable {
    ...
    private CompletionCursor completionCursor = new CompletionCursor(0);
    private long issuedChunks;
    ...
    public GenerationTask(final Chunky chunky, final Selection selection, final long count, final long time, final boolean cancelled) {
        ...
        this.completionCursor = new CompletionCursor(count);
        this.issuedChunks = count;
    ...
    public void run() {
        ...
        while (!stopped && chunkIterator.hasNext()) {
            ...
            isChunkGenerated
                    .thenCompose(generated -> {
                        ...
                        try {
                            if (throwable != null) {
                                // Never checkpoint past a failed chunk or report it as generated.
                                stop(false);
                                System.err.println("[Chunky] Generation failed at " + chunk + "; task paused for retry");
        ...
        // Pause/completion must retain the task until issued chunk requests finish.
        // The contiguous cursor also makes intermediate checkpoints safe when these
        // requests finish in a different order from their traversal positions.
        working.acquireUninterruptibly(MAX_WORKING_COUNT);
        working.release(MAX_WORKING_COUNT);
        if (stopped) {
