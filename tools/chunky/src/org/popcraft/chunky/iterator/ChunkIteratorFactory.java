...
public final class ChunkIteratorFactory {
    ...
    public static ChunkIterator getChunkIterator(final Selection selection, final long count) {
        ...
        if (TiledRectangleChunkIterator.PATTERN.equals(selection.pattern().getType())) {
            if (!ShapeType.RECTANGLE.equals(shape)) throw new IllegalArgumentException("Tile traversal requires rectangle");
            return new TiledRectangleChunkIterator(selection, count);
        }
